...
P1. Install AWS CLI in WSL + aws configure  (key=test, secret=test, region=us-east-1)
P2. Install LocalStack on Windows → copy to C:\bin → WSL PATH + wrapper → localstack --version
...
C.  until docker exec localstack curl -sf .../health | grep -q '"iot"'; do sleep 2; done
...
R3. Create iot-policy.json + aws iot create-policy    # see Real AWS IoT Core setup
R4. aws iot create-thing + create-keys-and-certificate + attach-policy + attach-principal
R5. Update secret.json: mqtt_broker, mqtt_ssl_verify=true, ca_cert, device_cert, device_key
...
# P2 — LocalStack on Windows (run in Windows CMD)
pip install localstack
copy "%LOCALAPPDATA%\Programs\Python\Python3x\Scripts\localstack.exe" C:\bin\
...
# Wait until IoT service is healthy (blocks until ready)
...
> Use `-e CAMERA_SOURCE=pattern` to skip `windows.camera.server` entirely (test pattern only).
...
==> Creating IoT policy: esp32p4-mqtt-policy
==> Creating certificate + private key...
    device.pem.crt  saved
    device.key      saved
    Certificate ARN: arn:aws:iot:us-east-1:...
...
explorer.exe "http://localhost:8080/stream"   # WSL2
# or: xdg-open http://localhost:8080/stream
```
...
```bash
sudo apt install mosquitto-clients   # if not already installed
...
```
devices/esp32p4-device-01/status     {"state":"online","chip":"esp32p4","emulator":true,...}
devices/esp32p4-device-01/telemetry  {"thing":"esp32p4-device-01","seq":0,"img_b":12345}
...
# Windows camera server: Ctrl-C in its CMD window, or close the window
```
...
| Changed | Path |
|---|---|
| `boot.py` or `main.py` only | [Python files only](#python-files-only-bootpy--mainpy) — no QEMU restart needed |
| `modcamera.c`, `sdkconfig.board`, `mpconfigboard.*` | [C code or board files](#c-code-or-board-files-modcamerac-sdkconfigboard-etc) — full recompile + restart |
| `secret.json` only | Re-upload with `mpremote` — see [Flash to physical ESP32-P4 hardware](#flash-to-physical-esp32-p4-hardware) |

### Python files only (boot.py / main.py)

Re-upload to the **running** emulator without restarting QEMU. The scripts
directory is bind-mounted into the emulator so `inject-scripts` picks up the
latest versions from `micropython/src/`:
...
A full recompile is needed. The emulator must be stopped, recreated with the
new firmware, and restarted.
...
`inject-scripts.py` is the **hot-reload tool** for the emulator. It uploads `.py` files into the running QEMU virtual device over the serial TCP port without restarting QEMU or rebuilding firmware. It is installed as the `inject-scripts` command inside the `esp32p4-emulator` Docker image.

Used in two ways:

| When | How |
|---|---|
| **Rebuild firmware — Python files only** | `docker exec esp32p4-emulator inject-scripts --host localhost --port 2323 --dir /scripts` |
| **Makefile shortcut** | `make upload-scripts` (same command via `docker exec`) |
...
│        Opens TCP connection to localhost:2323
│        Scans incoming bytes for ">>>" prompt
...
│        Skips secret.py (frozen in firmware — uploading it would shadow
│        the frozen version with an older copy)
│        Logs each upload; prints WARNING on non-zero exit
│
└── 3. Reset the device
         mpremote connect socket://localhost:2323 reset
         MicroPython reboots and runs the newly uploaded files
```

### Why it skips secret.py

`secret.py` is frozen into the firmware image by `manifest.py`. MicroPython resolves frozen modules before the filesystem, so uploading a `secret.py` to the flash would have no effect — the frozen copy always wins. `run-qemu.sh` explicitly skips it when copying files into the littlefs image for the same reason.

### Relationship to run-qemu.sh

`run-qemu.sh` bakes the initial scripts into the littlefs image before QEMU starts (no REPL needed). `inject-scripts.py` is used after QEMU is already running to update scripts without a full restart:

```
First boot:   run-qemu.sh  →  mklittlefs bakes scripts into flash  →  QEMU starts
                                                                              │
...
| `--timeout` | `60` | Seconds to wait for the REPL before giving up |
...
`setup-localstack.sh` provisions all required AWS IoT resources on a **running LocalStack container** and saves the device credentials to `certs/`. Run it once after the containers start — it is idempotent (safe to run again if resources already exist).

It is the equivalent of the real-AWS per-device steps but targeting `http://localhost:4566` instead of AWS.
...
Step 3 — Create certificate + key  aws iot create-keys-and-certificate --set-as-active
...
Step 6 — Extract LocalStack CA     openssl s_client → certs/ca.pem
                                   (optional — only needed if mqtt_ssl_verify=true)
```

### Output files

All saved to `certs/` (git-ignored):
...
| `certs/ca.pem` | LocalStack CA cert — only needed if `mqtt_ssl_verify=true` |
...
The dummy `test` credentials are required — LocalStack accepts any value but the AWS CLI rejects missing credentials. The `--endpoint-url=http://localhost:4566` flag is baked into the script.

### Idempotency

Every AWS CLI call is followed by `2>/dev/null || echo "(already exists)"`. Running the script twice creates a second certificate and attaches it alongside the first — both are valid. To start clean, delete all resources in LocalStack first (`docker rm -f localstack && docker volume rm localstack_data`).

### Relationship to real AWS setup

`setup-localstack.sh` targets LocalStack with `mqtt_ssl_verify=false` (self-signed cert). For real AWS, follow the [Real AWS IoT Core setup](#real-aws-iot-core-setup) section instead, which uses `aws iot create-thing` against real AWS and `mqtt_ssl_verify=true` with `AmazonRootCA1.pem`.

| | LocalStack | Real AWS |
|---|---|---|
| Endpoint | `http://localhost:4566` | `https://iot.<region>.amazonaws.com` |
| Credentials | `test` / `test` | Real IAM credentials |
| `mqtt_ssl_verify` | `false` | `true` |
| CA cert | `certs/ca.pem` (self-signed, extracted from LocalStack) | `AmazonRootCA1.pem` (downloaded from Amazon) |
| Script | `setup-localstack.sh` | Manual steps — see Real AWS IoT Core setup |
...
Files present inside each running container — baked-in vs bind-mounted at `docker run` time.

### `localstack` (official image — no custom Dockerfile)

```
/var/lib/localstack/          ← bind-mounted from localstack_data volume (persists IoT state)
/var/run/docker.sock          ← bind-mounted from host (LocalStack needs Docker for service spin-up)
```
...
Built from `Dockerfile.micropython` (builder stage). Contains the full IDF + MicroPython build tree:
...
Bind-mounted at `docker run` time (overlays the `COPY`'d versions with live host files):
```
...
/firmware-out/                                       ← firmware-out/ on host (output target)
```
...
Bind-mounted at `docker run` time:
```
...
/tmp/scripts/                 ← staging directory
  boot.py                     ← copied from /scripts/
  main.py                     ← copied from /scripts/
  secret.json                 ← generated by run-qemu.sh (merges /secret.json + emulator overrides)
```
...
micropython/
  ...
  src/
    ...
    main.py                         Camera capture + MQTT publish every 10 s; SSL on real hw
    secret.py                       Secret class — reads secret.json from device flash at runtime
...
firmware/
  main/
    mqtt_main.c                     ESP-IDF C reference implementation (not used by MicroPython)
    CMakeLists.txt
    Kconfig.projbuild
  CMakeLists.txt
